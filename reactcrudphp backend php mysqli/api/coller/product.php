    ...
    case "DELETE":
        ...
        case "PUT":
            $userUpdate= json_decode(file_get_contents("php://input"));
             ...
             $userid= $userUpdate->p_id;
             $username= $userUpdate->ptitle;
             $useremail= $userUpdate->pprice;
             $status= $userUpdate->pfile;
  
             $updateData= mysqli_query($db_conn, "UPDATE tbl_product SET ptitle='$username', pprice='$useremail', pfile='$status' WHERE p_id='$userid'  ");
             if($updateData)
             {
           ...
           // print_r($userUpdate); die;
            ...
            case "DELETE":
              ...
              $result= mysqli_query($db_conn, "DELETE FROM tbl_product WHERE p_id= '$path[4]' ");
              if($result)
              {
